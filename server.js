...
function autoSeedIfEmpty() {
  ...
  const sampleProjects = [
    { name: 'Ai小西算力扩充采购项目', procurement_type: '单一来源采购', created_by: '张三', createdAt: '2026-01-10T09:00:00', completedStages: 7 },
    { name: '安全围栏采购项目', procurement_type: '考察评估', created_by: '张三', createdAt: '2026-01-15T14:00:00', completedStages: 11 },
    { name: 'APP智能应用构建平台采购', procurement_type: '考察评估', created_by: '罗伯特', createdAt: '2026-02-20T10:30:00', completedStages: 4 },
    { name: '语音服务采购', procurement_type: '单一来源采购', created_by: '李四', createdAt: '2026-03-05T08:30:00', completedStages: 3 },
    { name: 'OA项目采购', procurement_type: '考察评估', created_by: '罗伯特', createdAt: '2026-03-18T15:00:00', completedStages: 9 },
    { name: '策略数据服务采购', procurement_type: '单一来源采购', created_by: '王五', createdAt: '2026-04-02T11:00:00', completedStages: 5 }
  ];

  const seedTxn = db.transaction(() => {
    for (const proj of sampleProjects) {
      const stages = DEFAULT_TEMPLATES[proj.procurement_type];
      if (!stages) continue;
      const snapshot = JSON.stringify(stages);
      const result = db.prepare(
        'INSERT INTO projects (name, procurement_type, created_by, created_at, template_snapshot) VALUES (?, ?, ?, ?, ?)'
      ).run(proj.name, proj.procurement_type, proj.created_by, proj.createdAt, snapshot);
      const projectId = result.lastInsertRowid;
      const projectDate = new Date(proj.createdAt);
      ...
      for (const stage of stages) {
        ...
        if (stage.order === 1) {
          status = 'completed';
          completedAt = projectDate.toISOString();
        } else if (stage.order <= proj.completedStages) {
          status = 'completed';
          let cumulativeDays = 0;
          for (let i = 0; i < stage.order; i++) cumulativeDays += stages[i].durationDays;
          const stageDate = new Date(projectDate);
          stageDate.setDate(stageDate.getDate() + cumulativeDays);
          completedAt = stageDate.toISOString();
        }
        db.prepare(
          'INSERT INTO project_stages (project_id, stage_order, status, completed_at) VALUES (?, ?, ?, ?)'
        ).run(projectId, stage.order, status, completedAt);
      }
    }

    const projectIds = {};
    for (const proj of sampleProjects) {
      const row = db.prepare('SELECT id FROM projects WHERE name = ?').get(proj.name);
      if (row) projectIds[proj.name] = row.id;
    }

    const shareInsert = db.prepare(
      'INSERT INTO project_shares (project_id, shared_by, shared_with, permission) VALUES (?, ?, ?, ?)'
    );
    shareInsert.run(projectIds['安全围栏采购项目'], '张三', '罗伯特', 'view');
    shareInsert.run(projectIds['Ai小西算力扩充采购项目'], '张三', '罗伯特', 'view');
    shareInsert.run(projectIds['语音服务采购'], '李四', '罗伯特', 'edit');
    shareInsert.run(projectIds['策略数据服务采购'], '王五', '罗伯特', 'view');
    shareInsert.run(projectIds['APP智能应用构建平台采购'], '罗伯特', '张三', 'edit');
    shareInsert.run(projectIds['OA项目采购'], '罗伯特', '李四', 'view');
  });

  seedTxn();
  console.log('✅ 已自动填充 6 个样例项目及分享记录');
}
autoSeedIfEmpty();
